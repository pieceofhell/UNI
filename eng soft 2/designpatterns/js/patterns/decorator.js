...
/**
 * Decorador para tornar a tarefa repetitiva diariamente
 */
class DailyRepeatDecorator extends TaskDecorator {
    constructor(task) {
        super(task);
    }

    getDescription() {
        return `${this.task.getDescription()} (Repetição diária)`;
    }

    getHtmlRepresentation() {
        const baseHtml = this.task.getHtmlRepresentation();
        return baseHtml.replace(
            '</div>',
            '<span class="badge bg-info ms-2">Repetição Diária</span></div>'
        );
    }
}
