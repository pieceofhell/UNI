...
class Task {
    ...
    return `
        <div class="list-group-item task-item" id="task-${this.id}">
            ...
            <small>Status: <strong>${this.getStatus()}</strong></small><br>
...
class HealthTask extends Task {
    ...
    getHtmlRepresentation() {
        const baseHtml = super.getHtmlRepresentation();
        return baseHtml.replace('list-group-item task-item', 'list-group-item task-item task-health');
    }
}
