...
double readNum(FILE* mignon){
  double a;
  fscanf(mignon, "%lf", &a);
  return a;
}

double* readLine(FILE* mignon){
  double* list = malloc(sizeof(double)*15);
  int isEnd = 0;
  char nextChar;
  int i = 0;
  while(!isEnd){
    fseek(mignon, -1, 1);
    *(list+i+1) = readNum(mignon);
    nextChar = (char)fgetc(mignon);
    if(nextChar == '\n'){
      isEnd = 1;
      fseek(mignon, -1, 1);
    }
...
void readEntireFile(FILE* mignon){
  int i = 0;
  while(fgetc(mignon) != EOF){
    double* a = readLine(mignon);
    if(i%2==0){
      //print(a);
      printf("\n");
      printf("%lf", func1(a));
...
int main(void) {
  FILE* mignon;
  mignon = fopen("testes.txt", "r");
  readEntireFile(mignon);
