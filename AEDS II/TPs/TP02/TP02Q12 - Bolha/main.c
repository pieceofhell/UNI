...
void bubbleSort(struct Jogador jogadoresSelecionados[], int tamanho)
{
    int i, j;
    struct Jogador temp;
    ...
    for (i = 0; i < tamanho - 1; i++)
    {
        ...
        {
            int comparacaoAno = strcmp(jogadoresSelecionados[j].anoNascimento, jogadoresSelecionados[j + 1].anoNascimento);
            comparacoes++;
            if (comparacaoAno > 0 || (comparacaoAno == 0 && strcmp(jogadoresSelecionados[j].nome, jogadoresSelecionados[j + 1].nome) > 0))
            {
                temp = jogadoresSelecionados[j];
                jogadoresSelecionados[j] = jogadoresSelecionados[j + 1];
                jogadoresSelecionados[j + 1] = temp;
                trocas++;
            }
        }
    }
...
int main()
{
    ...
    bubbleSort(jogadoresSelecionados, contadorSelecionados);
