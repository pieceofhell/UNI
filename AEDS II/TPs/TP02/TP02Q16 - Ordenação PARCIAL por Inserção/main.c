...
void insertionSort(struct Jogador jogadores[], int tamanho)
{
    for (int i = 1; i < tamanho; i++)
    {
        struct Jogador chave = jogadores[i];
        int j = i - 1;
        while (j >= 0 && strcmp(jogadores[j].anoNascimento, chave.anoNascimento) > 0)
        {
...
void ordenacaoParcialInsercao(struct Jogador jogadores[], int tamanho, int k)
{
    if (k <= 0 || k > tamanho)
    {
        k = tamanho;
    ...
    insertionSort(jogadores, tamanho);

    for (int i = 0; i < k; i++)
    {
        printf("[%d ## %s ## %d ## %.lf ## %s ## %s ## %s ## %s]\n",
               jogadores[i].id, jogadores[i].nome, jogadores[i].altura,
               jogadores[i].peso, jogadores[i].anoNascimento, jogadores[i].universidade,
               jogadores[i].cidadeNascimento, jogadores[i].estadoNascimento);
    }
}
...
int main()
{
    ...
    fptr = fopen("matricula_bubblesort.txt", "w");
    ...
    ordenacaoParcialInsercao(jogadoresSelecionados, contadorSelecionados, 10);
